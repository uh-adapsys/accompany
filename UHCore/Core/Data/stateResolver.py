...
class StateResolver(object):
    ...
    def evaluateRule(self, rule, value):
        ...
        try:
            ...
            print >> sys.stderr, 'Error parsing rule "%(rule)s" parsed:"%(pyrule)s", Exception: %(exception)s' % {
                'rule': rule, 'pyrule': pyRule, 'exception': e}
            return None
        ...
        for sensor in sensorList:
            ...
            if isOn is None or disp is None:
                if not "state_%s" % (sensor['sensorId'], ) not in self._warned:
                    print >> sys.stderr, 'Error parsing sensor "%(sensor)s" isOn:"%(isOn)s", state: %(state)s' % {
                        'sensor': sensor['name'], 'isOn': isOn, 'state': disp}
                continue
            else:
