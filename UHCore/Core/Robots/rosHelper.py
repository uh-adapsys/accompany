...
class ROS(object):
    ...
    def setParam(self, paramName, value, retry=5):
        ...
        with _threadLock:
            try:
                self._rospy.set_param(paramName, value)
            except CannotSendRequest:
                if retry > 0:
                    self.setParam(paramName, value, retry - 1)
                else:
...
class Transform(object):
    ...
    def transformPoint(self, point, fromTopic=None, toTopic=None):
        ...
        with _threadLock:
            ...
            try:
                ...
                # if str(e) != 'Unable to lookup transform, cache is empty, when looking
                # up transform from frame [' + baseTopic + '] to frame [' + mapTopic +
                # ']':
    ...
    def getTransform(self, fromTopic=None, toTopic=None):
        ...
        if len(self._ros.getTopics('base_pose', exactMatch=True)) == 0:
            ...
            (_, _, orientation) = self._tf.transformations.euler_from_quaternion(
