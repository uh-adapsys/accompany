...
testShowMe::testShowMe(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::testShowMe)
{
    ui->setupUi(this);


    ui->showMeTableWidget->clear();
    ui->showMeTableWidget->setColumnCount(2);

    ui->showMeTableWidget->verticalHeader()->setVisible(false);

    QStringList labs;
    labs << "Name" << "Probability";
    ui->showMeTableWidget->setHorizontalHeaderLabels(labs);

    testTimer = new QTimer();

    connect(testTimer, SIGNAL(timeout()), this, SLOT(doClassification()));

    testTimer->start(1000*testTiming);
}

void testShowMe::setTiming(int nTiming)
{

    testTiming = nTiming;

}
void testShowMe::doClassification()
{
    ...
    if (!query.exec(seqQuery))
     ...
     ui->showMeTableWidget->setRowCount(query.size());
    ...
    while (query.next())
    {
        ...
        ui->showMeTableWidget->setItem(row, 0, new QTableWidgetItem(query.value(0).toString()));
        ui->showMeTableWidget->item(row, 0)->setBackgroundColor(Qt::white);

        QTableWidgetItem *item = new QTableWidgetItem();
        item->setTextAlignment(Qt::AlignCenter);
        item->setText(sConf);
        ui->showMeTableWidget->setItem(row, 1, item);

        bool context = false;
        if (conf > 0.5)
        {
            ui->showMeTableWidget->item(row, 1)->setBackgroundColor(Qt::green);
            context = true;
        }
        else
        {
            ui->showMeTableWidget->item(row, 1)->setBackgroundColor(Qt::red);
        }

        if (!updateContextSensor(query.value(0).toString(),context)) return;

        ui->showMeTableWidget->resizeColumnsToContents();
...
testShowMe::~testShowMe()
{
    delete ui;
}


void testShowMe::createCase(QString classification)
{
...
float testShowMe::classify(QString item)
{
...
bool testShowMe::updateContextSensor(QString context, bool outcome)
{
    QSqlQuery query;
